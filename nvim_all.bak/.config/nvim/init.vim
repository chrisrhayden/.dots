...
"
" security:
" `nomodeline` should be set but there really isn't a good replacement for this
" functionality without crazy setup and config
"
" since `noexrc` is on by default so `secure` should not be necessary but i set
" both to on anyway
...
"   $XDG_DATA_HOME/nvim/{thing}
" but `backupdir` still try's for PWD
"
" see `:help vim-differences` in neovim for the differences

" cache:
" vim can make a lot of files (e.g. swap, undo, backup, etc,) so there is
" potential that it will fill up disk space, especially undo's i think.  maybe a
" cron/systemd job to clean up every now and then or if a directory gets to big

" tabstop:
" it sounds like you should be able to leave `tabstop` alone but there are so
" many edge cases that it doesn't really work well, along with this i want real
" tabs to be the equivalent amount of spaces visually so code and other text
" lines up well, so it becomes much easier to set `tabstop` and have the
" other tab settings like `expandtab` and `shiftwidth` use `tabstop`

" spell & thesaurus:
" spelling and thesaurus files can be found here if vim does not download them
" https://cgit.freedesktop.org/libreoffice/dictionaries/tree/en

" alternative settings:
" its very cumbersome to have alternative settings spread out,
" having settings in a central file like `web.vim` can help but still sucks

" sources:
" a lot of things in my rc come from stackoverflow but i didnt credit people at
" first, but i have started to when it seems appropriate
"
" sigh, i cant find where i got a lot of my mappings, I've also changed them a
" lot by now, i think an issues is the projects i think i got them from have
" changed, anyway i think its mostly these projects in no order
